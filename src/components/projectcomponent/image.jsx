...
const Image = () => {
    ...
    const animateTransition = (direction) => {
        ...
        // Exit animation - current image fades out with scale
        tl.to(currentImage, {
            scale: 0.8,
            opacity: 0,
            duration: 0.4,
        ...
        // Enter animation - new image fades in with scale
        .to(currentImage, {
            scale: 1,
            opacity: 1,
            duration: 0.4,
    ...
    useEffect(() => {
        // Initial animation on mount - only animate the centered image
        if (imageRef.current) {
    ...
    // Mouse wheel scroll handler - only when hovering over the image
    useEffect(() => {
        const handleWheel = (event) => {
            if (isAnimating) return; // Prevent scrolling during animation
        ...
        // Add wheel event listener only to the image element
    ...
    return (
        <>
            <section className='h-screen w-full relative overflow-hidden'>
                ...
                <img 
                    ref={bgImageRef}
                    src={currentProject.bgImg} 
                    className='absolute inset-0 w-full h-full object-cover brightness-50' 
                    alt="Background" 
                />
                
                {/* Logo and Pagination Header */}
                <div className="absolute top-0 left-0 right-0 z-10 flex justify-between items-center p-10">
                    <div className='flex flex-col justify-center items-center'>
                        ...
                        <p className='aeonikmed text-white'>Selected Project</p>
                    </div>
                    <div className="flex flex-col justify-center">
                        <span className='scroll text-3xl text-white'>{currentIndex + 1} / {totalImages}</span>
                    </div>
                </div>
                ...
                <div className='absolute inset-0 flex items-center justify-center z-10'>
                    <div className='text-center'>
                        <img 
                            ref={imageRef}
                            src={currentProject.frontImg} 
                            alt={currentProject.propertyName}
                            className='mx-auto mb-4 max-h-[50vh] object-contain'
                        />
                        <h2 className='scroll text-4xl text-white mb-2'>{currentProject.propertyName}</h2>
                        <p className='aeonikmed text-white text-xl'>{currentProject.propertyPlace}</p>
                    </div>
                </div>

                {/* Navigation */}
                <div className='absolute bottom-10 left-10 z-20 text-white text-xl'>
                    <span 
